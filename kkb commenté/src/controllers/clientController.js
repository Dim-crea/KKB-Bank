...
exports.addOperation = async (req, res) => {
    ...
    const idBody = req.body.compteEmetteur
    const newSolde = req.body.soldeApres

    await compteModel.getmodifySoldeCompte(idBody,newSolde).then(compteModel.creationCompteOperation(req.body)).then(() => {res.render("page/dashboard")}
    )
}
